...
def train(args):
    ...
    if "gpt" in model_name:
        tokenizer = TOKENIZER[model_name].from_pretrained(model_name, bos_token='<|startoftext|>', eos_token='<|endoftext|>', pad_token='<|pad|>')
        ...
        train_dataset=DeltaGenEncoderDecoderDataset(tokenizer, file_path=f"{data_dir}/train.jsonl", mode=INPUT_MODE[args.data_name],max_seq_len=args.max_seq_length)
        dev_dataset=DeltaGenEncoderDecoderDataset(tokenizer, file_path=f"{data_dir}/dev.jsonl", mode=INPUT_MODE[args.data_name],max_seq_len=args.max_seq_length)
